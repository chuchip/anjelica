...
/**
 * <p>Titulo: esqueletopad </p>
 * <p>Descripción: Esqueleto para Mantenimiento Tablas Simples. Esta basado en el
 * programa gnu.chu.anjelica.tesoreria.pdlibrovto
...
public abstract class esqueletopad extends ventanaPad implements PAD
{
  String s;
  boolean admin = false;
...
//  CButton Bcancelar = new CButton("Cancelar");
  ...
  {
    iniciarFrame();
    this.setSize(new Dimension(567, 312));
    this.setVersion("20050918 " + (admin ? "-ADMINISTRADOR-" : ""));
    ...
    Baceptar.setBounds(new Rectangle(123, 184, 126, 28));
    Bcancelar.setBounds(new Rectangle(301, 184, 126, 28));
    ...
    lbv_numfraE.setBounds(new Rectangle(84, 49, 53, 17));
    ...
    Pcabe.setBounds(new Rectangle(10, 4, 535, 71));
    ...
    Pdatos.setBounds(new Rectangle(12, 76, 531, 103));
...
//      debug("s: "+s);
  ...
  public boolean checkAddNew()
  {
    try {
      ...
      {
        mensajeErr("Introduzca Número de Ejercicio");
        eje_numeE.requestFocus();
        return false;
      }
      if (lbv_numfraE.getValorInt()==0)
      {
        mensajeErr("Introduzca Número de Factura");
        lbv_numfraE.requestFocus();
        return false;
      }
      if (lbv_numeE.getValorInt()==0)
      {
        mensajeErr("Introduzca Número de Vencimiento");
